...
import watermark_remover.backend.config as config
from watermark_remover.backend.inpaint.lama_inpaint import LamaInpaint
from watermark_remover.backend.inpaint.sttn_inpaint import STTNInpaint, STTNVideoInpaint
from watermark_remover.backend.inpaint.video_inpaint import VideoInpaint
from watermark_remover.backend.scenedetect import scene_detect
from watermark_remover.backend.scenedetect.detectors.content_detector import ContentDetector
from watermark_remover.backend.tools.common_tools import is_image_file, is_video_or_image
from watermark_remover.backend.tools.infer import utility
from watermark_remover.backend.tools.infer.predict_det import TextDetector
from watermark_remover.backend.tools.inpaint_tools import batch_generator, create_mask
...
class SubtitleDetect:
    ...
    def find_subtitle_frame_no(self, sub_remover=None):
        ...
        print("[Watermark Remover] Detecting watermark.")
        ...
        print("[Watermark Remover] Detected watermark.")
            ...
            for key in keys[1:]:
                ...
                for idx, region in enumerate(current_regions):
                    last_standard_region = (
                        unify_value_map[last_key][idx] if idx < len(unify_value_map[last_key]) else None
                    )
                ...
                for coord in subtitle_frame_no_box_dict[current_no]:
                    ...
                    if len(area_max_box_list) < 1:
                        area_max_box_list.append(
                            {"area": current_area, "xmin": xmin, "xmax": xmax, "ymin": ymin, "ymax": ymax}
                        )
                    # 如果列表非空，判断当前文本框是与区间最大文本框在同一区域
                        ...
                        for area_max_box in area_max_box_list:
                            if (
                                area_max_box["ymin"] - config.THRESHOLD_HEIGHT_DIFFERENCE <= ymin
                                ...
                                if (
                                    ...
                                    # 如果高度差异不一样
                                    if (
                                        abs(abs(area_max_box["ymax"] - area_max_box["ymin"]) - abs(ymax - ymin))
...
class SubtitleRemover:
    ...
    def sttn_mode_with_no_detection(self):
        ...
        if self.sub_area is not None:
            ...
            print(
                "[Watermark Remover] No watermark area has been set. Video will be processed in full screen. As a result, the final outcome might be suboptimal."
            )
    ...
    def run(self):
        ...
        if not self.is_picture:
            ...
            print(f"[Watermark Remover] Watermark removed, video generated at：{self.video_out_name}")
        else:
            print(f"[Watermark Remover] Watermark removed, picture generated at：{self.video_out_name}")
