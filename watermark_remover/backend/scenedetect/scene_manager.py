...
from backend.scenedetect.platform import Template, get_and_create_path, get_cv2_imwrite_params, tqdm
from backend.scenedetect.scene_detector import SceneDetector, SparseSceneDetector
from backend.scenedetect.stats_manager import FrameMetricRegistered, StatsManager
...
def write_scene_list(
    ...
    if include_cut_list:
        csv_writer.writerow(
            ["Timecode List:"] + cut_list if cut_list else [start.get_timecode() for start, _ in scene_list[1:]]
        )
    csv_writer.writerow(
        [
            "Scene Number",
...
def save_images(
    ...
    imwrite_param = [get_cv2_imwrite_params()[image_extension], encoder_param] if encoder_param is not None else []
            ...
            [
                ...
                for r in (
                    range(
                        start.get_frames(),
                        start.get_frames()
                        + max(1, end.get_frames() - start.get_frames()),  # guard against zero length scenes
                    )
        ...
        for j, image_timecode in enumerate(scene_timecodes):
            ...
            if frame_im is not None:
                ...
                if aspect_ratio is not None:
                    frame_im = cv2.resize(frame_im, (0, 0), fx=aspect_ratio, fy=1.0, interpolation=interpolation.value)
                ...
                if height or width:
                    ...
                    frame_im = cv2.resize(frame_im, (width, height), interpolation=interpolation.value)
                elif scale:
                    frame_im = cv2.resize(frame_im, (0, 0), fx=scale, fy=scale, interpolation=interpolation.value)
...
class SceneManager:
    ...
    def get_scene_list(
        ...
        scene_list = get_scenes_from_cuts(cut_list=cut_list, start_pos=self._start_pos, end_pos=self._last_pos + 1)
    ...
    def _get_event_list(self) -> List[Tuple[FrameTimecode, FrameTimecode]]:
        if not self._event_list:
            return []
        assert self._base_timecode is not None
        return [(self._base_timecode + start, self._base_timecode + end) for start, end in self._event_list]

    def _process_frame(
        self, frame_num: int, frame_im: np.ndarray, callback: Optional[Callable[[np.ndarray, int], None]] = None
    ) -> bool:
    ...
    def detect_scenes(
        ...
        while not self._stop.is_set():
            ...
            if progress_bar is not None:
                if new_cuts:
                    progress_bar.set_description(PROGRESS_BAR_DESCRIPTION % len(self._cutting_list), refresh=False)
            ...
            while not self._stop.is_set():
                ...
                if self._is_processing_required(video.position.frame_num):
                    ...
                    if downscale_factor > 1:
                        frame_im = cv2.resize(
                            frame_im,
                            (round(frame_im.shape[1] / downscale_factor), round(frame_im.shape[0] / downscale_factor)),
