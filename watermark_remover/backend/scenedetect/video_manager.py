...
class VideoParameterMismatch(Exception):
    ...
    def __init__(self, file_list=None, message="OpenCV VideoCapture object parameters do not match."):
...
def open_captures(
    ...
    if not is_device and any(
        [not os.path.exists(video_file) for video_file in video_files if not ("%" in video_file or "://" in video_file)]
    ):
    ...
    try:
        ...
        cap_frame_sizes = [
            (math.trunc(cap.get(cv2.CAP_PROP_FRAME_WIDTH)), math.trunc(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)))
            for cap in cap_list
        ]
...
def validate_capture_framerate(
    ...
    if framerate is not None:
        ...
        unavailable_framerates = [
            (video_names[i][0], video_names[i][1]) for i, fps in enumerate(cap_framerates) if fps < MAX_FPS_DELTA
        ]
...
def validate_capture_parameters(
    ...
    bad_params += [
        (cv2.CAP_PROP_FRAME_WIDTH, frame_size[0], cap_frame_sizes[0][0], video_names[i][0], video_names[i][1])
        for i, frame_size in enumerate(cap_frame_sizes)
        if abs(frame_size[0] - cap_frame_sizes[0][0]) > 0
    ]
    bad_params += [
        (cv2.CAP_PROP_FRAME_HEIGHT, frame_size[1], cap_frame_sizes[0][1], video_names[i][0], video_names[i][1])
        for i, frame_size in enumerate(cap_frame_sizes)
...
class VideoManager(VideoStream):
    ...
    def __init__(self, video_files: List[str], framerate: Optional[float] = None, logger=getLogger("pyscenedetect")):
        """[DEPRECATED] DO NOT USE.
