...
from scenedetect.platform import CommandTooLong, Template, get_ffmpeg_path, get_file_name, invoke_command, tqdm
...
def split_video_mkvmerge(
    ...
    try:
        ...
        if show_output:
            logger.info(
                "Average processing speed %.2f frames/sec.", float(total_frames) / (time.time() - processing_start_time)
            )
    except CommandTooLong:
        logger.error(COMMAND_TOO_LONG_STRING)
    except OSError:
        logger.error(
            "mkvmerge could not be found on the system." " Please install mkvmerge to enable video output support."
        )
    if ret_val != 0:
