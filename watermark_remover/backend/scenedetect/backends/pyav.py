...
class VideoStreamAv(VideoStream):
    ...
    def aspect_ratio(self) -> float:
        ...
        display_aspect_ratio = (
            self._codec_context.display_aspect_ratio.numerator / self._codec_context.display_aspect_ratio.denominator
        )
    ...
    def _get_duration(self) -> int:
        ...
        if duration_sec is None or duration_sec < MAX_FPS_DELTA:
            ...
            if time_base.denominator == 0:
                logger.warning(
                    "Unable to calculate video duration: time_base (%s) has zero denominator!", str(time_base)
                )
                return 0
