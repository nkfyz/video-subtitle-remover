...
import cv2
from backend.scenedetect.frame_timecode import MAX_FPS_DELTA, FrameTimecode
from backend.scenedetect.platform import get_file_name
from backend.scenedetect.video_stream import FrameRateUnavailable, SeekError, VideoOpenFailure, VideoStream
...
class VideoStreamCv2(VideoStream):
    ...
    def __init__(
        ...
        self._cap: Optional[cv2.VideoCapture] = None  # Reference to underlying cv2.VideoCapture object.
    ...
    def _open_capture(self, framerate: Optional[float] = None):
        ...
        if not cap.isOpened():
            raise VideoOpenFailure("Ensure file is valid video and system dependencies are up to date.\n")
