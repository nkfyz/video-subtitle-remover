...
from opensora_serving.plugins.watermark_remover.backend.scenedetect.detectors import ContentDetector
...
class AdaptiveDetector(ContentDetector):
    ...
    def process_frame(self, frame_num: int, frame_img: Optional[ndarray]) -> List[int]:
        ...
        average_window_score = sum(frame[1] for i, frame in enumerate(self._buffer) if i != self.window_width) / (
