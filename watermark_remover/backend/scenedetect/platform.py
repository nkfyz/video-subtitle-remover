...
def init_logger(log_level: int = logging.INFO, show_stdout: bool = False, log_file: Optional[str] = None):
    ...
    if show_stdout:
        ...
        handler.setFormatter(logging.Formatter(fmt=DEBUG_TEMPLATE if log_level == logging.DEBUG else INFO_TEMPLATE))
...
def get_system_version_info() -> str:
    ...
    for tool_name, tool_version in tool_version_info:
        out_lines.append(output_template.format(tool_name, tool_version if tool_version else not_found_str))
