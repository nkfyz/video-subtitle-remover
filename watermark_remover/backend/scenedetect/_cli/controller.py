...
def _save_stats(context: CliContext) -> None:
    ...
    if context.stats_file_path is not None:
        ...
        if context.stats_manager.is_save_required():
            logger.info("Saving frame metrics to stats file: %s", os.path.basename(context.stats_file_path))
...
def _list_scenes(
    context: CliContext, scene_list: List[Tuple[FrameTimecode, FrameTimecode]], cut_list: List[FrameTimecode]
) -> None:
    ...
    if context.scene_list_output:
        ...
        scene_list_path = get_and_create_path(
            scene_list_filename,
            context.scene_list_directory if context.scene_list_directory is not None else context.output_directory,
        )
...
""",
    ...
    if cut_list:
        logger.info("Comma-separated timecode list:\n  %s", ",".join([cut.get_timecode() for cut in cut_list]))
...
def _export_html(
    ...
    html_filename = Template(context.html_name_format).safe_substitute(VIDEO_NAME=context.video_stream.name)
    if not html_filename.lower().endswith(".html"):
        html_filename += ".html"
    html_path = get_and_create_path(
        html_filename, context.image_directory if context.image_directory is not None else context.output_directory
    )
...
def _split_video(context: CliContext, scene_list: List[Tuple[FrameTimecode, FrameTimecode]]) -> None:
    ...
    output_path_template = get_and_create_path(
        ...
        context.split_directory if context.split_directory is not None else context.output_directory,
    )
