...
class RangeValue(ValidatedValue):
    ...
    def from_config(config_value: str, default: "RangeValue") -> "RangeValue":
        try:
            ...
            raise OptionParseFailure("Value must be between %s and %s." % (default.min_val, default.max_val)) from ex
...
class ScoreWeightsValue(ValidatedValue):
    ...
    def __init__(self, value: Union[str, ContentDetector.Components]):
        if isinstance(value, ContentDetector.Components):
            ...
            translation_table = str.maketrans({char: " " for char in ScoreWeightsValue._IGNORE_CHARS})
        ...
        for option in CONFIG_MAP[command]:
            if command in config and option in config[command]:
                ...
                if issubclass(option_type, ValidatedValue):
                    try:
                        out_map[command][option] = option_type.from_config(config_value=config_value, default=default)
                    except OptionParseFailure as ex:
                        errors.append(
                            "Invalid [%s] value for %s:\n  %s\n%s" % (command, option, config_value, ex.error)
                        )
                    continue
...
class ConfigRegistry:
    ...
    def get_value(
        self, command: str, option: str, override: Optional[ConfigValue] = None, ignore_default: bool = False
    ) -> ConfigValue:
    ...
    def get_help_string(self, command: str, option: str, show_default: Optional[bool] = None) -> str:
        ...
        if show_default is False or (show_default is None and is_flag and CONFIG_MAP[command][option] is False):
            return ""
