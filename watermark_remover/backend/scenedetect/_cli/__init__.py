...
@click.option(
    ...
    % (", ".join(CHOICE_MAP["global"]["verbosity"]), USER_CONFIG.get_help_string("global", "verbosity")),
)
@click.option(
    "--logfile",
    "-l",
...
def help_command(ctx: click.Context, command_name: str):
    ...
    if command_name is not None:
        if not command_name in all_commands:
            error_strs = ["unknown command. List of valid commands:", "  %s" % ", ".join(sorted(all_commands))]
...
@click.option(
    ...
    type=click.FloatRange(
        CONFIG_MAP["scene_detect-content"]["threshold"].min_val, CONFIG_MAP["scene_detect-content"]["threshold"].max_val
    ),
    default=None,
...
@click.option(
    ...
    % (
        ""
        if USER_CONFIG.is_default("scene_detect-content", "min-scene-len")
        else USER_CONFIG.get_help_string("scene_detect-content", "min-scene-len")
    ),
)
@click.pass_context
def detect_content_command(
    ...
    detector_args = ctx.obj.get_detect_content_params(
        threshold=threshold, luma_only=luma_only, min_scene_len=min_scene_len, weights=weights, kernel_size=kernel_size
    )
...
@click.option(
    ...
    % (
        ""
        if USER_CONFIG.is_default("scene_detect-adaptive", "min-scene-len")
        else USER_CONFIG.get_help_string("scene_detect-adaptive", "min-scene-len")
    ),
)
@click.pass_context
...
@click.option(
    ...
    metavar="VAL",
    type=click.FloatRange(
        CONFIG_MAP["scene_detect-threshold"]["threshold"].min_val,
...
@click.option(
    ...
    metavar="PERCENT",
    type=click.FloatRange(
        CONFIG_MAP["scene_detect-threshold"]["fade-bias"].min_val,
...
@click.option(
    ...
    % (
        ""
        if USER_CONFIG.is_default("scene_detect-threshold", "min-scene-len")
        else USER_CONFIG.get_help_string("scene_detect-threshold", "min-scene-len")
    ),
)
@click.pass_context
...
@click.option(
    ...
    type=click.STRING,
    default=None,
    help="Name of column used to mark scene cuts.%s" % (USER_CONFIG.get_help_string("load-scenes", "start-col-name")),
)
@click.pass_context
...
@click.option(
    ...
    is_flag=True,
    flag_value=True,
    help="Only print scene list.%s" % (USER_CONFIG.get_help_string("list-scenes", "no-output-file")),
)
@click.option(
    "--quiet",
    "-q",
...
@click.option(
    ...
    is_flag=True,
    flag_value=True,
    help="Hide output from external video splitting tool.%s" % (USER_CONFIG.get_help_string("split-video", "quiet")),
)
@click.option(
    "--copy",
    "-c",
...
@click.option(
    ...
    % (", ".join(CHOICE_MAP["split-video"]["preset"]), USER_CONFIG.get_help_string("split-video", "preset")),
)
@click.option(
    "--args",
    "-a",
...
@click.option(
    ...
    default=None,
    type=click.INT,
    help="Height (pixels) of images.%s" % (USER_CONFIG.get_help_string("save-images", "height", show_default=False)),
)
@click.option(
    "--width",
    "-W",
    ...
    default=None,
    type=click.INT,
    help="Width (pixels) of images.%s" % (USER_CONFIG.get_help_string("save-images", "width", show_default=False)),
)
@click.pass_context
