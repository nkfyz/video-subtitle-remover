...
from scenedetect._cli.config import CHOICE_MAP, ConfigLoadFailure, ConfigRegistry
...
class CliContext:
    ...
    def __init__(self):
        ...
        self.default_detector: Tuple[Type[SceneDetector], Dict[str, Any]] = None  # [global] default-detector
    ...
    def handle_options(
        ...
        self.min_scene_len = parse_timecode(
            min_scene_len if min_scene_len is not None else self.config.get_value("global", "min-scene-len"),
            self.video_stream.frame_rate,
        )
        self.drop_short_scenes = drop_short_scenes or self.config.get_value("global", "drop-short-scenes")
        self.merge_last_scene = merge_last_scene or self.config.get_value("global", "merge-last-scene")
        ...
        scene_manager.interpolation = Interpolation[self.config.get_value("global", "downscale-method").upper()]
    ...
    def get_detect_adaptive_params(
        ...
        if not self.config.is_default("scene_detect-adaptive", "min-delta-hsv"):
            logger.error(
                "[scene_detect-adaptive] config file option `min-delta-hsv` is deprecated"
                ", use `min-delta-hsv` instead."
            )
            if self.config.is_default("scene_detect-adaptive", "min-content-val"):
                self.config.config_dict["scene_detect-adaptive"]["min-content-val"] = self.config.config_dict[
        ...
        if weights is not None:
            try:
                ...
                raise click.BadParameter(str(ex), param_hint="weights")
        return {
            "adaptive_threshold": self.config.get_value("scene_detect-adaptive", "threshold", threshold),
            "weights": self.config.get_value("scene_detect-adaptive", "weights", weights),
            "kernel_size": self.config.get_value("scene_detect-adaptive", "kernel-size", kernel_size),
            "luma_only": luma_only or self.config.get_value("scene_detect-adaptive", "luma-only"),
            "min_content_val": self.config.get_value("scene_detect-adaptive", "min-content-val", min_content_val),
            "min_scene_len": min_scene_len,
            "window_width": self.config.get_value("scene_detect-adaptive", "frame-window", frame_window),
        }
    ...
    def get_detect_threshold_params(
        ...
        return {
            ...
            "add_final_scene": add_last_scene or self.config.get_value("scene_detect-threshold", "add-last-scene"),
            "fade_bias": self.config.get_value("scene_detect-threshold", "fade-bias", fade_bias),
            "min_scene_len": min_scene_len,
            "threshold": self.config.get_value("scene_detect-threshold", "threshold", threshold),
        }
    ...
    def handle_load_scenes(self, input: AnyStr, start_col_name: Optional[str]):
        ...
        self.add_detector(
            SceneLoader(file=input, framerate=self.video_stream.frame_rate, start_col_name=start_col_name)
        )
    ...
    def handle_list_scenes(
        ...
        self.scene_list_directory = self.config.get_value("list-scenes", "output", output, ignore_default=True)
    ...
    def handle_split_video(
        ...
        self.split_directory = self.config.get_value("split-video", "output", output, ignore_default=True)
        ...
        if mkvmerge or copy:
            ...
            if high_quality:
                raise click.BadParameter(
                    "high-quality (-hq) cannot be used with %s" % (command), param_hint="split-video"
                )
            if args:
                raise click.BadParameter("args (-a) cannot be used with %s" % (command), param_hint="split-video")
            if rate_factor:
                raise click.BadParameter(
                    "rate-factor (crf) cannot be used with %s" % (command), param_hint="split-video"
                )
            if preset:
                raise click.BadParameter("preset (-p) cannot be used with %s" % (command), param_hint="split-video")
    ...
    def handle_save_images(
        ...
        self.scale_method = Interpolation[self.config.get_value("save-images", "scale-method").upper()]
        ...
        self.image_directory = self.config.get_value("save-images", "output", output, ignore_default=True)
    ...
    def _open_video_stream(self, input_path: AnyStr, framerate: Optional[float], backend: Optional[str]):
        ...
        try:
            if backend is None:
                ...
                if not backend in AVAILABLE_BACKENDS:
                    raise click.BadParameter(
                        "Specified backend %s is not available on this system!" % backend, param_hint="-b/--backend"
                    )
            ...
            if backend == "pyav":
                ...
                self.video_stream = open_video(
                    ...
                    framerate=framerate,
                    backend=backend,
                    max_decode_attempts=self.config.get_value("backend-opencv", "max-decode-attempts"),
                )
            ...
            raise click.BadParameter(
                "Failed to open input video%s: %s" % (" using %s backend" % backend if backend else "", str(ex)),
                param_hint="-i/--input",
            ) from ex
