...
class FrameMetricNotRegistered(Exception):
    ...
    def __init__(
        self, metric_key: str, message: str = "Attempted to get/set frame metrics for unregistered metric key."
    ):
        super().__init__(message)
        self.metric_key = metric_key
...
class StatsManager:
    ...
    def save_to_csv(
        self, csv_file: Union[str, bytes, TextIO], base_timecode: Optional[FrameTimecode] = None, force_save=True
    ) -> None:
        ...
        if not ((self.is_save_required() or force_save) and self._registered_metrics and self._frame_metrics):
    ...
    def _metric_exists(self, frame_number: int, metric_key: str) -> bool:
        return frame_number in self._frame_metrics and metric_key in self._frame_metrics[frame_number]
