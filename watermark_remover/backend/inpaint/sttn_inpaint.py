...
from watermark_remover.backend.inpaint.sttn.auto_sttn import InpaintGenerator
from watermark_remover.backend.inpaint.utils.sttn_utils import Stack, ToTorchFormatTensor

# 定义图像预处理方式
_to_tensors = transforms.Compose([Stack(), ToTorchFormatTensor()])  # 将图像堆叠为序列  # 将堆叠的图像转化为PyTorch张量
...
class STTNInpaint:
            ...
            for j in range(len(frames_hr)):
                ...
                for k in range(len(inpaint_area)):
                    ...
                    frame[inpaint_area[k][0] : inpaint_area[k][1], :, :] = (
                        mask_area * comp + (1 - mask_area) * frame[inpaint_area[k][0] : inpaint_area[k][1], :, :]
                    )
                # 将最终帧添加到列表
    ...
    def inpaint(self, frames: List[np.ndarray]):
        ...
        for f in range(0, frame_length, self.neighbor_stride):
            # 计算邻近帧的ID
            neighbor_ids = [
                i for i in range(max(0, f - self.neighbor_stride), min(frame_length, f + self.neighbor_stride + 1))
            ]
            # 获取参考帧的索引
