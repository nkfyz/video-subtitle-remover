...
from watermark_remover.backend import config
from watermark_remover.backend.inpaint.video.core.utils import to_tensors
from watermark_remover.backend.inpaint.video.model.misc import get_device
from watermark_remover.backend.inpaint.video.model.modules.flow_comp_raft import RAFT_bi
from watermark_remover.backend.inpaint.video.model.propainter import InpaintGenerator
from watermark_remover.backend.inpaint.video.model.recurrent_flow_completion import (
...
class VideoInpaint:
    def __init__(self, sub_video_length=config.PROPAINTER_MAX_LOAD_NUM, use_fp16=True, model_path=None):
        ...
        self.ref_stride = 10
        # 设置raft模型
        self.fix_raft = self.init_raft_model(model_path)
        # 设置fix_flow模型
        self.fix_flow_complete = self.init_fix_flow_model()
        # 设置inpaint模型
        self.model = self.init_inpaint_model()

    def init_raft_model(self, model_path=None):
        # set up RAFT and flow competition model
        return RAFT_bi(os.path.join(model_path, "raft-things.pth"), self.device)
    ...
    def init_inpaint_model(self):
        ...
        return (
            InpaintGenerator(model_path=os.path.join(config.VIDEO_INPAINT_MODEL_PATH, "ProPainter.pth"))
            .to(self.device)
            .eval()
        )
    ...
    def inpaint(self, frames, mask):
        ...
        flow_masks, masks_dilated = read_mask(
            mask, frames_len, size, flow_mask_dilates=self.mask_dilation, mask_dilates=self.mask_dilation
        )
        w, h = size
