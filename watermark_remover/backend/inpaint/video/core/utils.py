...
def read_dirnames_under_root(root_dir):
    dirnames = [
        name for i, name in enumerate(sorted(os.listdir(root_dir))) if os.path.isdir(os.path.join(root_dir, name))
    ]
    ...
    for _ in range(video_length - 1):
        ...
        if extra_transform > 0.75:
            ...
            region = region.resize(
                (math.ceil(region_width * resize_coefficient), math.ceil(region_height * resize_coefficient)),
                Image.NEAREST,
            )
            m.paste(region, (y, x, y + region.size[0], x + region.size[1]))
            region_width, region_height = region.size
...
def get_random_shape(edge_num=9, ratio=0.7, width=432, height=240):
    ...
    verts = (
        np.stack((np.cos(angles), np.sin(angles))).T * (2 * ratio * np.random.random(points_num) + 1 - ratio)[:, None]
    )
    verts[-1, :] = verts[0, :]
...
def random_move_control_points(
    X, Y, imageHeight, imageWidth, lineVelocity, region_size, maxLineAcceleration=(3, 0.5), maxInitSpeed=3
):
...
if __name__ == "__main__":
    ...
    for _ in range(trials):
        ...
        masks = create_random_shape_with_random_motion(video_length, imageHeight=240, imageWidth=432)
