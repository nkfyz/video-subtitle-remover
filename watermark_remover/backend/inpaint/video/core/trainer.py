...
class Trainer:
        ...
        if scheduler_type in ["MultiStepLR", "MultiStepRestartLR"]:
            ...
            if not self.config["model"]["no_dis"]:
                self.scheD = MultiStepRestartLR(
                    self.optimD, milestones=scheduler_opt["milestones"], gamma=scheduler_opt["gamma"]
                )
    ...
    def _train_epoch(self, pbar):
        ...
        while train_data is not None:
            ...
            # ---- image propagation ----
            prop_imgs, updated_local_masks = self.netG.module.img_propagation(
                masked_local_frames, pred_flows_bi, local_masks, interpolation=self.interp_mode
            )
            updated_masks = masks.clone()
            updated_masks[:, :l_t, ...] = updated_local_masks.view(b, l_t, 1, h, w)
            updated_frames = masked_frames.clone()
            prop_local_frames = (
                gt_local_frames * (1 - local_masks) + prop_imgs.view(b, l_t, 3, h, w) * local_masks
            )  # merge
            ...
            if self.config["losses"]["perceptual_weight"] > 0:
                perc_loss = (
                    self.perc_loss(pred_imgs.view(-1, 3, h, w), frames.view(-1, 3, h, w))[0]
            ...
            if self.iteration % 200 == 0:
                ...
                if masked_local_frames.shape[1] > 5:
                    ...
                    masked_flows_forward_cpu = (gt_flows_forward_cpu[0] * (1 - local_masks[0][0].cpu())).to(
                        gt_flows_forward_cpu
                    )
                    pred_flows_forward_cpu = flow_to_image(pred_flows_bi[0][0]).cpu()

                    flow_results = torch.cat(
                        [gt_flows_forward_cpu[0], masked_flows_forward_cpu, pred_flows_forward_cpu[0]], 1
                    )
                    if self.gen_writer is not None:
            ...
            if self.config["global_rank"] == 0:
                ...
                if not self.config["model"]["no_dis"]:
                    ...
                    pbar.set_description((f"hole: {hole_loss.item():.3f}; " f"valid: {valid_loss.item():.3f}"))
