...
from core.lr_scheduler import CosineAnnealingRestartLR, MultiStepRestartLR
from core.prefetch_dataloader import CPUPrefetcher, PrefetchDataLoader

# from skimage.feature import canny
from model.canny.canny_filter import Canny
from model.modules.flow_comp_raft import EdgeLoss, FlowLoss, RAFT_bi
...
class Trainer:
    def __init__(self, config):
        ...
        if config["distributed"]:
            self.train_sampler = DistributedSampler(
                self.train_dataset, num_replicas=config["world_size"], rank=config["global_rank"]
            )
    ...
    def setup_optimizers(self):
        ...
        self.optimG = torch.optim.Adam(
            optim_params, betas=(self.config["trainer"]["beta1"], self.config["trainer"]["beta2"])
        )
    ...
    def setup_schedulers(self):
        ...
        if scheduler_type in ["MultiStepLR", "MultiStepRestartLR"]:
            self.scheG = MultiStepRestartLR(
                self.optimG, milestones=scheduler_opt["milestones"], gamma=scheduler_opt["gamma"]
            )
        elif scheduler_type == "CosineAnnealingRestartLR":
            self.scheG = CosineAnnealingRestartLR(
                self.optimG, periods=scheduler_opt["periods"], restart_weights=scheduler_opt["restart_weights"]
            )
        else:
    ...
    def _train_epoch(self, pbar):
        ...
        while train_data is not None:
            ...
            if self.iteration % 200 == 0 and self.gen_writer is not None:
                ...
                masked_flows_forward_cpu = (gt_flows_forward_cpu[t] * (1 - local_masks[0][t].cpu())).to(
                    gt_flows_forward_cpu
                )
                pred_flows_forward_cpu = flow_to_image(pred_flows_bi[0][0]).cpu()

                flow_results = torch.cat(
                    [gt_flows_forward_cpu[t], masked_flows_forward_cpu, pred_flows_forward_cpu[t]], 1
                )
                ...
                masked_flows_backward_cpu = (gt_flows_backward_cpu[t] * (1 - local_masks[0][t + 1].cpu())).to(
                    gt_flows_backward_cpu
                )
                pred_flows_backward_cpu = flow_to_image(pred_flows_bi[1][0]).cpu()

                flow_results = torch.cat(
                    [gt_flows_backward_cpu[t], masked_flows_backward_cpu, pred_flows_backward_cpu[t]], 1
                )
                ...
                masked_edges_forward_cpu = (gt_edges_forward_cpu[t] * (1 - local_masks[0][t].cpu())).to(
                    gt_edges_forward_cpu
                )
                pred_edges_forward_cpu = pred_edges_bi[0][0].cpu()

                edge_results = torch.cat(
                    [gt_edges_forward_cpu[t], masked_edges_forward_cpu, pred_edges_forward_cpu[t]], 1
                )
                self.gen_writer.add_image("img/edge-f:gt-pred", edge_results, self.iteration)
                # backward
                gt_edges_backward_cpu = gt_edges_bi[1][0].cpu()
                masked_edges_backward_cpu = (gt_edges_backward_cpu[t] * (1 - local_masks[0][t + 1].cpu())).to(
                    gt_edges_backward_cpu
                )
                pred_edges_backward_cpu = pred_edges_bi[1][0].cpu()

                edge_results = torch.cat(
                    [gt_edges_backward_cpu[t], masked_edges_backward_cpu, pred_edges_backward_cpu[t]], 1
                )
            ...
            if self.config["global_rank"] == 0:
                ...
                if self.iteration % self.train_args["log_freq"] == 0:
                    logging.info(
                        f"[Iter {self.iteration}] " f"flow: {flow_loss.item():.4f}; " f"warp: {warp_loss.item():.4f}"
                    )
