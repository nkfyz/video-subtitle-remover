...
def ternary_loss(flow_comp, flow_gt, mask, current_frame, shift_frame, scale_factor=1):
    ...
    noc_mask = torch.exp(-50.0 * torch.sum(torch.abs(current_frame - warped_sc), dim=1).pow(2)).unsqueeze(1)
...
class EdgeLoss(nn.Module):
    ...
    def forward(self, pred_edges, gt_edges, masks):
        ...
        for i in range(len(pred_edges)):
            ...
            edge_loss = edgeLoss(pred_edges[i].reshape(-1, 1, h, w), gt_edges[i].reshape(-1, 1, h, w)) + 5 * edgeLoss(
