...
from backend.inpaint.video.model.modules.sparse_transformer import SoftComp, SoftSplit, TemporalSparseTransformerBlock
...
class Discriminator_2D(BaseNetwork):
    def __init__(self, in_channels=3, use_sigmoid=False, use_spectral_norm=True, init_weights=True):
        ...
        self.conv = nn.Sequential(
            ...
            # nn.InstanceNorm2d(256, track_running_stats=False),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv3d(nf * 4, nf * 4, kernel_size=(1, 5, 5), stride=(1, 2, 2), padding=(0, 2, 2)),
        )
